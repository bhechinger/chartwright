...
**Architecture:** The workspace has three crates: `chartwright-runtime` renders embedded chart files, `chartwright-abi` defines the dynamic module ABI, and `chartwright-cli` generates chart crates. Generated crates embed chart files and export ABI functions backed by the runtime.
...
**Files:**
- Create: `Cargo.toml`
- Create: `crates/chartwright-runtime/Cargo.toml`
- Create: `crates/chartwright-runtime/src/lib.rs`
- Create: `crates/chartwright-abi/Cargo.toml`
- Create: `crates/chartwright-abi/src/lib.rs`
- Create: `crates/chartwright-cli/Cargo.toml`
- Create: `crates/chartwright-cli/src/main.rs`
...
[workspace]
members = [
  "crates/chartwright-abi",
  "crates/chartwright-runtime",
  "crates/chartwright-cli",
]
resolver = "2"
...
repository = "https://example.invalid/chartwright"
...
Create minimal manifests for the three crates. `chartwright-runtime` depends on `serde`, `serde_json`, `serde_yaml`, and `thiserror`. `chartwright-abi` depends on `serde` and `serde_json`, with dev-dependencies `libloading` and `tempfile`. `chartwright-cli` depends on `clap`, `chartwright-runtime`, `serde_json`, `serde_yaml`, and `thiserror`, with dev-dependency `tempfile`.
...
**Files:**
- Modify: `crates/chartwright-runtime/src/lib.rs`
- Create: `crates/chartwright-runtime/tests/basic_render.rs`
...
Run: `cargo test -p chartwright-runtime --test basic_render renders_basic_chart`
...
Run: `cargo test -p chartwright-runtime --test basic_render`

### Task 3: ABI Contract

**Files:**
- Modify: `crates/chartwright-abi/src/lib.rs`
- Create: `crates/chartwright-abi/tests/buffer.rs`
...
Run: `cargo test -p chartwright-abi --test buffer`
...
- Create: `crates/chartwright-cli/tests/import.rs`

- [ ] **Step 1: Write a failing CLI import test.**

The test runs the CLI logic against `fixtures/basic-chart`, writes to a temp directory, and asserts generated `Cargo.toml` and `src/lib.rs` exist and contain `crate-type = ["cdylib", "rlib"]`, `chartwright_render_json`, and embedded chart files.

- [ ] **Step 2: Run the CLI test and confirm it fails because importer logic does not exist.**

Run: `cargo test -p chartwright-cli --test import`

- [ ] **Step 3: Implement importer logic.**

Parse `chartwright import <chart-dir> --crate-dir <out-dir>`, validate `Chart.yaml`, collect files under `templates/`, and generate a crate with embedded chart files and ABI exports.
...
The test imports `fixtures/basic-chart` into a temp crate, runs `cargo build --release` in that crate, loads the produced dynamic library with `libloading`, calls `chartwright_render_json`, reads the returned buffer, frees it, and compares output to `fixtures/basic-chart/golden.yaml`.
